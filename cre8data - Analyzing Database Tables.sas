************************************************************************;
* Program creates two fake tables in the specified location            *;
* - LOANS_RAW - Variety of loans for each customer                     *;
* - CUSTOMERS - Fake customer information                              *;
************************************************************************;
...
* - Set number of Customers : Total number of rows = (specified number) x (number of available threads) *;
* - The LOANS_RAW table will be about 3x as big as the number of customers                              *;
%let numLoanCustomers = 620123; /*Demo uses 620123 */

* Set the output caslib *;
%let outputCaslib = orac_cas;
...
* Set output caslib *;
* Modify the data source to create the fake data in your environments *;
...
proc cas;
    source ds;
        data &outputCaslib..loans_raw(drop=SalaryGroup Date initialCurrentCategories CurrentCategories Email CurrentDate Salary	EmpLength Age) /*accounts*/
             &outputCaslib..customers(keep=CurrentCategories ID Salary SalaryGroup Date Age EmpLength Email); /*customer info */
            call streaminit(99);
            ...
            ****************************************************;
            * 2. Set the current date for consistency: DEC2024 *;
            ****************************************************;
            * Make character *;
...
*****************************************************************************************************************;
                ...
                ******************************************************;
                  ...
                  * Output to loans raw for each customer and all accounts *;  
                   LastPurchase=DHMS(LastPurchase,rand('uniform',1,24),rand('uniform',1,60),0); 
                   output &outputCaslib..loans_raw;
            ...
            * Outputs the last row created for the customer to the customers table *;
            email = cats(ID,'@email.com');
            output &outputCaslib..customers;
            ...
            * Reset the lists that hold each category of loan that customers has *;
            ...
            format LastPurchase Date date9.;
        run;

    endsource;

* Execute the data step from above *; 
...
proc cas;
    do castbl over {'LOANS_RAW', 'CUSTOMERS'};
       table.save / 
            table  = {name = castbl, caslib = "&outputCaslib"},
            name = castbl, caslib = "&outputCaslib", replace = TRUE,
            dataSourceoptions= {insertbuff=3200};
    end;
